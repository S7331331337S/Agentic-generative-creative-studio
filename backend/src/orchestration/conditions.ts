...
/**
 * Resolves a `steps.<stepId>.<field>` reference against the results collected so
 * far. A reference to a step that has not produced a result yet resolves to
 * undefined, which compares unequal to everything.
 */
...
export function evaluateCondition(
  ...
  function parseOr(): boolean {
    ...
    while (peek() === '||') {
      next();
      // No short-circuit: the right side must still parse, so typos surface.
