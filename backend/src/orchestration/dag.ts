 ...
 */
export function validateSteps(steps: WorkflowStep[]): void {
  if (steps.length === 0) {
  ...
  for (const step of steps) {
    ...
    if (step.condition) {
      // Throws ConditionSyntaxError on malformed guards.
...
function detectCycle(steps: WorkflowStep[], byId: Map<string, WorkflowStep>): void {
  const UNVISITED = 0;
  const IN_PROGRESS = 1;
  const DONE = 2;
 ...
 * its siblings, so a level can be executed concurrently. Assumes validateSteps
 * has already run.
 */
export function toExecutionLevels(steps: WorkflowStep[]): WorkflowStep[][] {
